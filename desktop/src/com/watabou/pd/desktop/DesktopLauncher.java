...
public class DesktopLauncher {
	public static void main (String[] arg) {
		...
		if (SharedLibraryLoader.isMac) {
			config.preferencesDirectory = "Library/Application Support/3oiDburg Pixel Dungeon/";
		} else if (SharedLibraryLoader.isLinux) {
			config.preferencesDirectory = ".3oiDburg/wuwuwu/";
		} else if (SharedLibraryLoader.isWindows) {
			String winVer = System.getProperties().getProperty("os.name");
			if (winVer.contains("XP")) {
				config.preferencesDirectory = "Application Data/.3oiDburg/wuwuwu/";
			} else {
				config.preferencesDirectory = "AppData/Roaming/.3oiDburgwuwuwu/";
			}
		}
		...
		config.title = "3oiDburg Pixel Dungeon";
